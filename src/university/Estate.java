...
/**
 * The estate class manages different facilities, facility capacities, and a data record for the properties of
 * different types of facilities.
 * It also can search for the most suitable facilities/buildings to upgrade or build.
 */
public class Estate {
  ...
  /**
   * called by the EcsSim tries to find a building of the type with the maximum level, then do the
   * upgrade on it
   *
   * @return null if no building of such type is in facilities or if not enough funds to upgrade
   *     any/all are at max level, or the building to perform the upgrade passed back to the caller
   *     (university)
   */
  public Building findBuildingToUpgrade(String type, float availableFunds) {
    if (availableFunds <= 0) return null;
    // prioritizes availableFunds for upgrading the building with the highest capacity, as graphs
    // from excel showed me that
    // buildings with higher capacity/level tend to have higher capacity increase/upgrade cost
    // ratio, thus more "lucrative".
    PriorityQueue<AbstractBuilding> pq = new PriorityQueue<>();
    for (Facility facility : facilities) {
      if (facility
          .getClass()
          .getSimpleName()
          .equals(type)) // easier mapping from string type name to class
      {
  ...
  /**
   * Maps a String type to a concrete class Does an additional check for budget deficit and adds to
   * the facility arraylist
   *
  ...
  /**
   * Simple maps a String type to an object of a concrete class does not add to the facilities
   * arraylist
   *
   * @return a Facility/Building/AbstractBuilding whatever! denoted by the String type
   */
  ...
  /**
   * Construct a new facility. The new facility is specified by its type, e.g., "Hall", "Lab",
   * "Theatre", and its name. The new facility should be added to the facilities list. In the case
   * where the type is not known, null is returned.
   */
  ...
  /**
   * Return the yearly maintenance course of the facilities. In particular, for a building, the cost
   * of maintenance (in ECScoins) is equivalent to 10% of the building’s capacity.
   */
  ...
  public String updateCapacities() {
    ...
    // this default hall > theatre > lab order helps with deciding which building to prioritize
    // building
